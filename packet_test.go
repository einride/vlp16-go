...
func BenchmarkPacket_ReadFrom_Example(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		if err := packet.Read(bytes.NewReader(exampleData[:])); err != nil {
			b.Fatal(err)
		}
	}
}
...
func TestPacket_ReadFrom_Example(t *testing.T) {
	...
	exampleData := getExampleData()
	err := actual.Read(bytes.NewReader(exampleData[:]))
	require.NoError(t, err)
	require.Equal(t, getExamplePacket(), actual)
}

func getExamplePacket() vlp16.Packet {
	return vlp16.Packet{
...
func getExampleData() [1206]uint8 {
	return [1206]uint8{

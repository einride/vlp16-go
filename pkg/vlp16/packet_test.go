...
func TestPacket_ReadFrom_TestData(t *testing.T) {
	...
	for {
		_, err = packet.ReadFrom(testData)
		if err == io.EOF {
			break
		} else if err != nil {
			t.Fatal(err)
		...
		numPackets++
		require.Equal(t, vlp16.ReturnModeStrongest, packet.ReturnMode)
		require.Equal(t, vlp16.ProductIDVLP16, packet.ProductID)
		require.True(t,
...
func BenchmarkPacket_ReadFrom_Example(b *testing.B) {
	var packet vlp16.Packet
	for i := 0; i < b.N; i++ {
		if _, err := packet.ReadFrom(bytes.NewReader(exampleData[:])); err != nil {
			b.Fatal(err)
		}
	}
}

func TestPacket_ReadFrom_Example(t *testing.T) {
	var actual vlp16.Packet
	_, err := actual.ReadFrom(bytes.NewReader(exampleData[:]))
	require.NoError(t, err)
	require.Equal(t, examplePacket, actual)
}

var examplePacket = vlp16.Packet{
